...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (isDev) {
      console.log("[OMR_COMPARE] payload recibido", {
        answerKeyLength: answerKey?.length ?? 0,
        studentAnswersLength: studentAnswers?.length ?? 0,
        exigencia,
      })
      console.log("[OMR_COMPARE] correctAnswers (primeras 15)", JSON.stringify((answerKey ?? []).slice(0, 15)))
      console.log("[OMR_COMPARE] studentAnswers (primeras 15)", JSON.stringify((studentAnswers ?? []).slice(0, 15)))
    }
    ...
    // Crear mapa de respuestas del profesor (normalizado: number, mayúscula, trim)
    const profesorMap = new Map<number, string>()
    for (const item of answerKey) {
      const num = Number(item.pregunta)
      if (Number.isFinite(num)) {
        profesorMap.set(num, String(item.respuestaCorrecta ?? "").trim().toUpperCase())
      }
    }

    // Crear mapa de respuestas del estudiante (normalizado: number, mayúscula, trim)
    ...
    for (const item of studentAnswers) {
      ...
      if (Number.isFinite(num)) {
        estudianteMap.set(num, {
          respuesta: String(item.respuesta ?? "").trim().toUpperCase() || "SIN_RESPUESTA",
    ...
    if (isDev) {
      console.log("[OMR_COMPARE] resultado calculado", {
        ...
        nota: result.nota,
        requierenRevisionCount: result.requierenRevision.length,
      })
      console.log("[OMR_COMPARE] primeras 10 resultados", result.resultados.slice(0, 10))
    }
