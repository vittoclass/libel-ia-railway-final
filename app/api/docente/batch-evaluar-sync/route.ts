...
export async function GET(req: NextRequest) {
  ...
  const includeEvaluatedPhotos = req.nextUrl.searchParams.get("include_evaluated_photos") === "1"
  ...
  let photosForEvaluar = withUrls
  if (!includeEvaluatedPhotos) {
    ...
    photosForEvaluar = withUrls.filter((photo) => {
